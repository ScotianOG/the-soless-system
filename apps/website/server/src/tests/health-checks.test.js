...
describe("System Health Checks", () => {
    ...
    describe("Core Services Health", () => {
        ...
        it("should load configuration properly", async () => {
            ...
            const contestConfig = ConfigManager_1.configManager.getContestConfig();
            const pointsConfig = ConfigManager_1.configManager.getPointsConfig();
            const telegramConfig = ConfigManager_1.configManager.getPlatformConfig("TELEGRAM");
        ...
        it("should handle service singleton patterns correctly", async () => {
            ...
            const rewardManager1 = RewardManager_1.RewardManager.getInstance();
            const rewardManager2 = RewardManager_1.RewardManager.getInstance();
            expect(rewardManager1).toBe(rewardManager2);
            const pointManager1 = PointManager_1.PointManager.getInstance();
            const pointManager2 = PointManager_1.PointManager.getInstance();
            expect(pointManager1).toBe(pointManager2);
            const userManager1 = UserManager_1.UserManager.getInstance();
            const userManager2 = UserManager_1.UserManager.getInstance();
    ...
    describe("Performance Benchmarks", () => {
        ...
        it("should handle concurrent operations efficiently", async () => {
            ...
            console.log(`📊 Concurrent Operations: ${concurrentOperations} ops in ${totalTime}ms (avg: ${avgTime.toFixed(2)}ms)`);
    ...
    describe("Error Recovery", () => {
            ...
            const errorTests = [
                {
                    ...
                    test: () => prisma.user.findUnique({ where: { id: "invalid-uuid-format" } }),
                },
                {
                    name: "Malformed query",
                    test: () => prisma.engagement
                        .findMany({
                        where: {
        ...
        it("should maintain system stability under stress", async () => {
            ...
            const stressTest = async () => {
                ...
                for (let i = 0; i < 20; i++) {
                    operations.push(prisma.user.count(), prisma.engagement.count(), prisma.contest.findFirst());
                }
            ...
            console.log(`📊 Stress Test: 60 operations in ${duration}ms, post-test health check: ${healthCheckTime}ms`);
    ...
    describe("Resource Usage", () => {
        ...
        it("should handle connection pooling efficiently", async () => {
            ...
            for (let i = 0; i < 20; i++) {
                connectionTests.push(prisma.user.count(), prisma.engagement.count(), prisma.contest.count());
            }
            const startTime = Date.now();
            ...
            console.log(`📊 Connection Pool Test: ${connectionTests.length} operations in ${duration}ms`);
