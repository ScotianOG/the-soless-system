...
exports.getTimeSeriesStats = getTimeSeriesStats;
async function getTimeSeriesStats(prisma, timeFrame = "WEEK") {
  ...
  `;
    ...
    if (Array.isArray(engagementData)) {
        engagementData.forEach((item) => {
            ...
            if (item.platform in statsObj) {
                statsObj[item.platform].engagements = Number(item.count);
    ...
    if (Array.isArray(pointsData)) {
        pointsData.forEach((item) => {
            ...
            const statsObj = timeSeriesMap.get(dateKey);
            if (item.platform in statsObj) {
                statsObj[item.platform].points = Number(item.points);
