...
exports.twitterAuthRouter = void 0;
...
const handleTwitterCallback = async (req, res) => {
    try {
      ...
      `);
        ...
        const result = await authService.handleTwitterCallback(code, wallet);
    ...
    `);
        ...
        logger_1.logger.error("Twitter callback error:", error);
        res.setHeader("Content-Type", "text/html");
        res.send(`
      <script>
        window.opener.postMessage(
...
const handleTwitterAuth = async (req, res) => {
    try {
        ...
        logger_1.logger.error("Twitter auth error:", error);
...
exports.twitterAuthRouter = router;
